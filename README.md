...
| **耗材库房**        | 整卷克重 / 剩余库存 / 低库存预警；打印记录自动扣减、删除加回；支持补货；支持拓竹局域网 / 云端 AMS 一键同步（见下）                                                                                              |
...
- Node/Docker 部署下静态资源为协商缓存（no-cache），Service Worker 抓取为网络优先：**普通刷新（F5）即可拿到最新前端**，无需 Ctrl+F5 或刷新两次。
- 「保存即刷新」：Node 服务端监听 `index.html` / `sw.js` / `assets/**` 变更，文件一保存，已打开的页面经 SSE（`/__reload`）自动重载。设环境变量 `NO_WATCH=1` 可关闭。Cloudflare Workers / Vercel 宿主无该通道（页面会静默停用，退回普通刷新）。
...
「设置 → 数据与账号管理 → 拓竹连接」中配置连接后，「耗材」页的 **拓竹同步** 卡片可从拓竹（Bambu Lab）打印机的 AMS 自动读取料卷的品牌 / 类型 / 颜色 / 剩余克数，一键合并进耗材库。纯只读：仅订阅打印机状态上报，不下发任何控制指令。

连接方式**二选一**：顶部切换「局域网直连 / 拓竹云」，选中哪个模式就只显示和只使用哪个连接（另一种模式的配置会保留，切回来即可继续用）：

| 方式 | 凭据 | 说明 |
| ---- | ---- | ---- |
| **局域网** | 打印机 IP + 局域网访问码 | 打印机屏幕「设置 → 局域网」查看访问码。服务端经 MQTT over TLS（8883 端口，用户名 `bblp`）直连打印机，支持 X1 / P1 / A1 系列。**要求本平台服务与打印机在同一局域网**（Docker / NAS / 裸跑 Node 部署） |
| **拓竹云** | 手机短信 或 账号密码，或 accessToken | 登录方式二选一（同拓竹设备管理类应用）：**手机短信**=手机号 + 获取验证码 + 验证并登录；**账号密码**=账号 + 密码（被要求验证时自动切到短信方式填码，2FA 账号填动态验证码）。区域支持中国大陆 / 欧洲 / 北美。受拓竹人机验证风控影响登录失败时，可直接粘贴 accessToken 兜底 |

同步逻辑：

1. 点「立即同步」抓取所有已配置来源的 AMS 快照（AMS 四托盘 + 外部料盘）；
2. 预览逐条显示匹配结果：优先按 RFID `tray_uuid` 匹配，其次按「类型 + 颜色」匹配（类型支持 PLA ↔ PLA Basic 这类前缀匹配）；
3. 勾选「更新已匹配耗材的库存」和 / 或「新增未匹配的耗材」后应用；新建耗材自动带出品牌 / 类型 / 颜色 / 整卷克重，单价需手动补充。

安全与隐私：

- 访问码、accessToken、云密码**只保存在服务端**（每用户私有集合 `bambucfg`，不进入 `/api/data`、导出备份与管理员 all-data），接口只回传「是否已配置」的脱敏视图；
- 多用户模式下每个账号维护自己的连接配置，同步写入自己的耗材库；
- Cloudflare Workers / Vercel 等无局域网环境：拓竹云方式可能可用，局域网方式不可用（无本地网络），建议使用 Docker / NAS / 裸跑 Node 部署。
