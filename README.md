...
「耗材」页内置 **拓竹同步** 卡片，可从拓竹（Bambu Lab）打印机的 AMS 自动读取料卷的品牌 / 类型 / 颜色 / 剩余克数，一键合并进耗材库。纯只读：仅订阅打印机状态上报，不下发任何控制指令。
...
| **拓竹云** | 账号密码 或 accessToken | 自动识别中国大陆 / 欧洲 / 北美节点；开启验证的账号会走邮箱验证码 / 2FA。也可直接粘贴 accessToken（中国区账号若不支持密码登录请用此方式）。token 约 24 小时有效，过期后重新登录即可 |
