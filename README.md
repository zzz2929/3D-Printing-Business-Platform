...
「设置 → 数据与账号管理 → 拓竹连接」卡片完成连接配置与数据同步：点「获取设备与耗材」后，**发现的打印机**（型号 / 序列号 / 功率预估）与**发现的耗材**（AMS 料卷品牌 / 类型 / 颜色 / 剩余克数）分区预览，分别一键同步到打印机库与耗材库。纯只读：仅订阅打印机状态上报，不下发任何控制指令。
...
0. 打印机：按「型号 → 设备名」匹配已有打印机；未匹配的可新建（品牌自动填拓竹，功率按型号经验预估，电价 / 购入价等手动补充）；
1. 耗材：抓取所有已配置来源的 AMS 快照（AMS 四托盘 + 外部料盘）；
